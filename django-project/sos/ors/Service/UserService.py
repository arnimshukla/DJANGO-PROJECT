...
class UserService:
    ...
    def addmarksheet(self, data):
        ...
        sql = "insert into marksheet values((%s), (%s), (%s), (%s), (%s), (%s), (%s))"
        data = [UserService.nextPkk(self), f, l, e, p, d, a]
        cursor = connection.cursor()
        cursor.execute(sql, data)
        connection.commit()
        connection.close()
    ...
    def getmarkes(self, id):
        sql = "select * from marksheet where id = (%s)"
        data = [id]
        cursor = connection.cursor()
        cursor.execute(sql, data)
        result = cursor.fetchall()
        columnName = ("id", "firstName", "Class", "studentId", "physics", "chemistry", "maths")
        res = []
        for x in result:
            print({columnName[i]: x[i] for i, _ in enumerate(x)})
            res.append({columnName[i]: x[i] for i, _ in enumerate(x)})
    ...
    def updatemarksheet(self, data):
        ...
        sql = "update marksheet set firstName = (%s), Class  = (%s), studentId   = (%s), physics     = (%s),chemistry    = (%s),maths    = (%s) where id     = (%s)"
        data = [f, l, e, p, d, a, i]
        cursor = connection.cursor()
        cursor.execute(sql, data)
        connection.commit()
